...
def create_reg_filter(regularizer):
    
    regularizer_type = regularizer[0][0]
    regularization_freq = regularizer[0][1]
    
    if regularizer_type == 'gauss':
        kernel_size = (regularizer[1],regularizer[1])
        sigma = (regularizer[2],regularizer[2])
        filter = kornia.filters.GaussianBlur2d(kernel_size, sigma)
        
    if regularizer_type == 'box':
        kernel_size = (regularizer[1],regularizer[1])
        filter = kornia.filters.BoxBlur(kernel_size)
        
    if regularizer_type == 'median':
        kernel_size = (regularizer[1],regularizer[1])
        filter = kornia.filters.MedianBlur(kernel_size)
           
        
    if regularizer_type == 'laplacian':
        kernel_size = regularizer[1]
        filter = kornia.filters.Laplacian(kernel_size)
 
    if regularizer_type == 'sobel':
        filter = kornia.filters.Sobel()

        
    return filter,regularization_freq
    
def dream(network,labels,image_dim,mean,std,nItr=100,lr=0.1,random_seed=0,regularizer=[('gauss',1),3,0.5]):
    ...
    filter,filter_apply_freq = create_reg_filter(regularizer)
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    network,filter = move_network_to_device(network,filter,device)
    
    image_tensor = dream_kernel(network,filter,filter_apply_freq,image_dim,labels,mean,std,nItr,lr,random_seed,device)

    return image_tensor

def dream_kernel(network,filter,filter_apply_freq,image_dim,labels,mean,std,nItr,lr,random_seed,device):
    ...
    for i in range(nItr):
        ...
        if i % filter_apply_freq == 0:
            #start = time.time()
            img.data = filter(img.data)
            #end = time.time()
