...
def main():
    ...
    train_type_list = ["../models/net_2000_0.001_200_0.0.pth","../models/net_2000_0.001_400_0.0.pth"]
    ...
    if cmd_args.change_lr:
        for lr in lr_list:
            ...
            name = "_".join(word for word in words) + str(lr) + ".png"
    ...
    if cmd_args.change_random_seed:
        for rs in rs_list:
            ...
            name = "_".join(word for word in words) + str(rs) + ".png"
            name = os.path.join(output_path,name)
            dreamer.show(im,name)
    ...
    if cmd_args.change_nItr:
        for nItr in nItr_list:
            ...
            name = "_".join(word for word in words) + str(nItr) + ".png"
            name = os.path.join(output_path,name)
            dreamer.show(im,name)
    ...
    if cmd_args.change_sigma:
        for sigma in sigma_list:
            ...
            name = "_".join(word for word in words) + str(sigma) + ".png"
            name = os.path.join(output_path,name)
            dreamer.show(im,name)
    ...
    if cmd_args.change_network:
        for model_name in model_list:
            ...
            name = str(num_labels) + "_".join(word for word in words) + ".png"
            name = os.path.join(output_path,name)
            dreamer.show(im,name)
